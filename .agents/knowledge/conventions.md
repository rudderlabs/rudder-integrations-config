...
- CustomerIO `apiVersion` defaults to `v2` in both `ui-config.json` and `schema.json` so new destinations are set up on the unified batch API. Because the validator runs ajv with `useDefaults: true` (`src/validator/index.ts`), this default is _written into_ any config that omits `apiVersion` — absent no longer means v1. Existing connections therefore depend on the config-be migration script backfilling an explicit `apiVersion: "v1"`; that migration must land before this default reaches production, and any connection it misses will validate as v2 without tripping the `allOf` guard (the guard's `if` requires `apiVersion` to be present, which an injected default does not satisfy).
