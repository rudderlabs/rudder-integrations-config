...
- OpenAI Ads event-filtering UI prerequisites should reference `eventFilteringOption` without a `.web` qualifier; keep the UI group visible for either cloud mode or web device mode and avoid web-device-only copy for these destination-wide fields.
