...
## ACT2-860 — BigQuery Source WIF Account Metadata

- BigQuery source account metadata represents its two supported methods with the descriptive `authenticationType` label `service_account_key_or_workload_identity_federation`; non-OAuth multi-auth source accounts conventionally use underscore-delimited `_or_` labels such as `password_or_iam` and `pat_or_oauth`.
- Keep the existing derived Project ID and Service account summary fields for keyed BigQuery source accounts, but gate them—along with credentials—on `authMethod == serviceAccountKey`. WIF continues to use the existing required `project` account option, while credential-derived summaries must not render or become required for keyless WIF accounts.
