...
## DEX-677 — Confluent Cloud Destination Boundary

- `confluent_cloud` already has an authored destination definition under `src/configurations/destinations/confluent_cloud/`; within `rudder-integrations-config`, treat the existing `db-config.json`/`ui-config.json`/`schema.json` triplet as the source of truth rather than scaffolding or rewriting it for CLI-only onboarding work.
- The `onboard-destination` CLI surface is outside this repository; integrations-config should not receive config-only or agentic documentation edits when the requested onboarding target is the CLI and the destination definition already exists.
