...
## INT-7143 — Google Ads Offline Conversions Consent Schema Tightening

- Google Ads Offline Conversions had baseline schema-generator drift for `consentManagement`: the generator wanted `items.required: ["provider"]` on each source-type consent-management array, matching the UI's required provider field and unique row behavior.
- Fixing that drift during the Form Builder V2 migration is a customer-visible validation tightening for malformed saved consent rows, so PR notes should call it out separately from the mechanical UI-config migration.
