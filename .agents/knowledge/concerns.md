...
- New destination archive fields emitted by this repository have a dependency boundary with `rudder-config-backend`; the three `destination_definitions` columns (`version`/`fallbackVersion`/`versions`) must exist there before this deploy change runs against an environment, since the payload now always carries them. Deploy ordering is CBE-columns-first, then these versioned definitions.
