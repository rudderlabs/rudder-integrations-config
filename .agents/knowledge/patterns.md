...
- In the WIF branch, define `secret` with empty `properties` and `additionalProperties: false` so combined-schema validation rejects service-account credentials. The account form can still submit values from hidden fields until the webapp clears them, so UI hiding alone does not guarantee secret removal.
